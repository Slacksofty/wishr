...
pub async fn create_item_list(name: String) -> Result<i64, ServerFnError> {
    ...
    let result = sqlx::query(
        "INSERT INTO item_lists (name, created_at) VALUES (?, ?)"
    )
    .bind(name.trim().to_string())
    .bind(now)
    .execute(&pool)
...
pub async fn get_wish_item(id: i64) -> Result<WishItem, ServerFnError> {
    ...
    let item = sqlx::query_as::<_, WishItem>(
...
pub async fn create_wish_item(
    ...
    .bind(want_level)
    .bind(need_level)
    .bind(if where_to_buy.is_empty() { None } else { Some(where_to_buy) })
...
pub async fn get_archive() -> Result<Vec<ArchivedItem>, ServerFnError> {
    ...
    #[derive(sqlx::FromRow)]
    struct ArchivedRow {
        w_id: i64, w_list_id: i64, w_name: String, w_description: Option<String>,
        w_estimated_cost: Option<f64>, w_want_level: i64, w_need_level: i64,
        w_where_to_buy: Option<String>, w_category: Option<String>,
        w_notes: Option<String>, w_status: String, w_created_at: String,
        p_id: i64, p_item_id: i64, p_actual_cost: Option<f64>,
        p_purchased_at: String, p_notes: Option<String>,
    }
    ...
    let archived = rows
        ...
        .map(|r| ArchivedItem {
            item: WishItem {
                id: r.w_id, list_id: r.w_list_id, name: r.w_name,
                description: r.w_description, estimated_cost: r.w_estimated_cost,
                want_level: r.w_want_level, need_level: r.w_need_level,
                where_to_buy: r.w_where_to_buy, category: r.w_category,
                notes: r.w_notes, status: r.w_status, created_at: r.w_created_at,
            },
            record: PurchaseRecord {
                id: r.p_id, item_id: r.p_item_id, actual_cost: r.p_actual_cost,
                purchased_at: r.p_purchased_at, notes: r.p_notes,
            },
        })
        .collect();
...
pub async fn get_list_stats(list_id: i64) -> Result<WishStats, ServerFnError> {
    ...
    Ok(WishStats { total_spent, pending_budget, active_count, purchased_count })
}
...
pub async fn get_stats() -> Result<WishStats, ServerFnError> {
    ...
    let total_spent: f64 = sqlx::query_scalar(
        "SELECT COALESCE(SUM(actual_cost), 0.0) FROM purchase_records"
    )
    .fetch_one(&pool)
    .await
    .map_err(|e| ServerFnError::new(e.to_string()))?;
    ...
    let active_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM wish_items WHERE status = 'active'"
    )
    .fetch_one(&pool)
    .await
    .map_err(|e| ServerFnError::new(e.to_string()))?;
    ...
    let purchased_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM wish_items WHERE status = 'purchased'"
    )
    .fetch_one(&pool)
    .await
    .map_err(|e| ServerFnError::new(e.to_string()))?;
